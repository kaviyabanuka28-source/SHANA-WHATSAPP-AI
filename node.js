...
// ========== ඔක්කොම EVENTS මුලින්ම REGISTER කරන්න ==========
...
// ==================== *** AUTO-REPLY HANDLER *** ====================
// ONLY 'message' event එක use කරන්න — මේක incoming messages වලට විතරයි
client.on('message', async (message) => {
    // Debug: message එක ලැබුනද කියලා check කරන්න
    console.log(`📨 MSG RECEIVED | From: ${message.from} | Body: "${message.body}"`);
    
    // Bot විසින්ම යවපු messages skip කරන්න
    if (message.fromMe) {
    ...
    console.log(`📝 Processing message from ${userId}: "${msg}"`);

    // Cooldown check
    if (!canReply(userId)) {
        console.log(`⏳ Cooldown for ${userId}, skipping`);
        return;
    }
    ...
    // Send reply (message.reply() වෙනුවට client.sendMessage() use කරන්න)
    if (replyText) {
        const success = await sendReply(userId, replyText);
        if (success) {
            userCooldowns.set(userId, Date.now());
            console.log(`✅ Auto-reply completed for ${userId}`);
        }
    }
});
